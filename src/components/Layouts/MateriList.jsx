const MateriList = ({ headline, materiList, linkRef }) => {
    return (
        <div className="bg-white shadow-md shadow-slate-500/30 rounded-[20px] py-6 px-10" ref={linkRef}>
            <h1 className="text-[#2B3E58] text-[30px] font-bold">{headline}</h1>
            <ul className="mt-5">
                {
                    materiList.map((list, index) => (
                        <li key={index} className="py-2">
                            <div className="flex gap-2 items-center">
                                ...
                                <h3 className="font-[600] text-[18px]">{list.subject}</h3>
                            </div>
